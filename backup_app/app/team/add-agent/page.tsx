...
export default async function AddAgentPage() {
  ...
  } else if (agentsCount && subscriptionData.subscription_plans?.[0]?.max_agents && 
            agentsCount >= subscriptionData.subscription_plans[0].max_agents) {
    limitMessage = `הגעת למגבלת הנציגים (${subscriptionData.subscription_plans[0].max_agents}). אנא שדרג את המנוי כדי להוסיף נציגים נוספים.`
  } else {
    canAddAgent = true
  }
        ...
        {canAddAgent ? (
          <>
            <p className="mb-6 text-gray-600">
              ...
              {subscriptionData?.subscription_plans?.[0]?.max_agents && (
                <span className="mr-2">
                  נותרו {subscriptionData.subscription_plans[0].max_agents - (agentsCount || 0)} מתוך {subscriptionData.subscription_plans[0].max_agents} נציגים במנוי שלך.
                </span>
              )}
            </p>
