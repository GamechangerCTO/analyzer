...
export default function SimulationsDashboard({ userId, userRole, companyId }: SimulationsDashboardProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'active': return 'bg-brand-info-light text-brand-primary-dark'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-2xl font-bold text-brand-info">{stats.thisWeek}</p>
        </div>
      </div>
      ...
      {userRole === 'agent' && (
        <div className="bg-gradient-to-r from-brand-primary to-brand-info rounded-xl p-6 text-white">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-lg font-bold mb-2">🎯 מוכן לאימון חדש?</h3>
              <p className="text-brand-info-light">
                התחל סימולציה חדשה ושפר את כישורי המכירות שלך
              </p>
            </div>
            <Link 
              href="/simulations/create"
              className="bg-white text-brand-primary px-6 py-3 rounded-lg font-medium hover:bg-brand-info-light transition-colors"
            >
              התחל אימון 🚀
            </Link>
          </div>
                    ...
                    <div className="flex flex-col gap-2">
                      {simulation.status === 'completed' && (
                        <Link
                          ...
                          className="px-3 py-1 bg-brand-info-light text-brand-primary-dark rounded text-sm hover:bg-brand-info-light transition-colors text-center"
                        >
                          צפה בדוח
                        </Link>
                      )}
                      ...
                      {simulation.status === 'pending' && (
                        <Link
                          ...
                          className="px-3 py-1 bg-brand-success-light text-brand-success-dark rounded text-sm hover:bg-brand-success-light transition-colors text-center"
                        >
                          המשך
                        </Link>
                      )}
                ...
                {simulation.simulation_reports_hebrew?.[0] && (
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {simulation.simulation_reports_hebrew[0].strengths.length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium text-brand-success-dark mb-1">נקודות חזקות:</h4>
